...
description: >-
  ...
  (default), MiniMax (key only), Volcengine Doubao, SiliconFlow ASR. Use when user shares
  xiaoyuzhoufm.com/episode link or asks for 小宇宙播客逐字稿/转写/字幕. Requires API Key from
  one provider. After install, prompt user for aliyun/minimax/doubao/siliconflow key.
license: MIT
...
| 后端 | 可转写 | 说明 |
|------|--------|------|
| **aliyun** | 是（默认） | 阿里云百炼 DashScope `fun-asr`，公网 URL 直转 |
| **doubao** | 是 | 火山引擎豆包「大模型录音文件极速版」 |
| **siliconflow** | 是 | 硅基流动 SenseVoice / TeleSpeech |
| **minimax** | 否（仅存 Key） | MiniMax 开放平台暂无私开录音转写 API，可先配置备用 |
...
bash {baseDir}/scripts/configure.sh minimax sk-用户的Key
...
bash {baseDir}/scripts/configure.sh status    # 查看四后端配置状态
```
...
| `minimax_api_key` / `minimax_group_id` | MiniMax（可选） |
| `volcengine_*` | 豆包 |
| `siliconflow_*` | 硅基流动 |
...
| default=minimax 无法转写 | `configure.sh default aliyun` |
